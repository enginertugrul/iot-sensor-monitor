...
import com.enginertugrul.iotsensormonitor.dto.reading.SensorDailyStatisticDTO;
...
import com.enginertugrul.iotsensormonitor.repository.DailySensorStatisticProjection;
import com.enginertugrul.iotsensormonitor.repository.HourlySensorStatisticProjection;
...
public class SensorReadingServiceImpl implements SensorReadingService {
    ...
    @Override
    @Transactional(readOnly = true)
    public SensorStatisticsDTO getStatistics(Long sensorId, Long ownerId, TemperatureUnit temperatureUnit) {
        ...
        return new SensorStatisticsDTO(
                ...
                today,
                getLastSevenDays(sensor, today, temperatureUnit),
                getHourlyStatistics(sensor, today, temperatureUnit)
        );
    }
    ...
    getHourlyStatisticsForDate(Long sensorId, Long ownerId, LocalDate date, TemperatureUnit temperatureUnit) {

        Sensor sensor = getOwnedSensor(sensorId, ownerId);

        return getHourlyStatistics(sensor, date, temperatureUnit);
    }
    ...
    private List<SensorDailyStatisticDTO> getLastSevenDays(Sensor sensor, LocalDate today, TemperatureUnit temperatureUnit) {

        ZoneId zoneId = ZoneId.of(sensor.getTimezone());

        Instant startInclusive = today.minusDays(6).atStartOfDay(zoneId).toInstant();
        ...
        if (sensor.getType() == SensorType.MOTION) {
            databaseResults = sensorReadingRepository.findDailyMotionDetectionCounts(
                                    sensor.getId(),
                                    startInclusive,
                                    endExclusive,
                                    sensor.getTimezone()
            ...
            databaseResults = sensorReadingRepository.findDailyNumericStatistics(
                            sensor.getId(),
        ...
        Map<LocalDate, Double> valuesByDate = databaseResults.stream().collect(Collectors.toMap(
                                DailySensorStatisticProjection::getDate,
                                result -> toDisplayValue(sensor.getType(), result.getValue(), temperatureUnit)
                        ));

        List<SensorDailyStatisticDTO> result = new ArrayList<>(7);
        ...
        for (int daysAgo = 6; daysAgo >= 0; daysAgo--) {
            ...
            if (value == null && sensor.getType() == SensorType.MOTION) {
                value = 0.0;
            }

            result.add(new SensorDailyStatisticDTO(date, value));
        }
    ...
    private List<SensorHourlyStatisticDTO> getHourlyStatistics(Sensor sensor, LocalDate date, TemperatureUnit temperatureUnit) {
        ...
        if (sensor.getType() == SensorType.MOTION) {
            databaseResults =sensorReadingRepository.findHourlyMotionDetectionCounts(
                                    sensor.getId(),
                                    startInclusive,
                                    endExclusive,
                                    sensor.getTimezone()
            ...
            databaseResults = sensorReadingRepository.findHourlyNumericStatistics(
                                    sensor.getId(),
                                    getCanonicalUnit(sensor.getType()).name(),
                                    startInclusive,
                                    endExclusive,
        ...
        Map<Short, Double> valuesByHour = databaseResults.stream().collect(Collectors.toMap(
                                HourlySensorStatisticProjection::getHour,
                                result -> toDisplayValue(sensor.getType(), result.getValue(), temperatureUnit)
                        ));

        List<SensorHourlyStatisticDTO> result = new ArrayList<>(24);
        ...
        for (short hour = 0; hour < 24; hour++) {
            ...
            result.add(new SensorHourlyStatisticDTO(hour, value));
        }
    ...
    private MeasurementUnit getCanonicalUnit(SensorType sensorType) {

        return switch (sensorType) {
            case TEMPERATURE -> MeasurementUnit.C;
            case HUMIDITY -> MeasurementUnit.PERCENT;
            case MOTION -> throw new IllegalArgumentException("Motion sensors do not have a numeric unit");
        };
    }
